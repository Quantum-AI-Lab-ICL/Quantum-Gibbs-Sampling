function [] = observable_specific_mixing_time()
    ...
    %%%%%%%%%% relative error epsilon on the spectral norm %%%%%%%%
    tolerance = 0.01;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%



    mixing_time = LindbladianMixingTime(beta,H,n,tolerance,observable, jumps = "Paulis", filter = "Gaussian", alsoEvaluateGap = false);
...
function [mixingtime] = LindbladianMixingTime(beta,H,n,tolerance,observable,options)
    ...
    function [finalvector] = evaluateAction(vector)
        ...
        for newi = 1:n
            ...
            for newt = 1:type_max
                ...
                M = M + ctranspose(newL) * sigma * newL;
    ...
    SteadyVector = eye(2^n,2^n);
    SteadyVector = reshape(SteadyVector,[2^(2*n),1]);

    if(verbosity)
        check = max(abs(evaluateAction(SteadyVector)),[],"all") %%%%% ideally should be 0
    end
    ...
    o_max = eigs(observable,1,"largestreal");
    o_min = eigs(observable,1,"smallestreal");
    ...
    ExpectationValue = trace(observable*GibbsMatrix);

    
%%%%%% calculating the evolved observable using small time steps and series expansions at each step


    error_on_observable = tolerance * norm(full(observable));
    ...
    observable = reshape(transpose(observable),[2^(2*n),1]);
    Evolution = observable;

    while condition > error_on_observable

        mixingtime = mixingtime + timestep;

        times = [times, mixingtime];
        ...
        o_max = eigs(EvolvedMatrix,1,"largestreal");
        o_min = eigs(EvolvedMatrix,1,"smallestreal");

        condition = max([o_max - ExpectationValue,ExpectationValue - o_min]);

        eig_max = [eig_max, o_max];
        eig_min = [eig_min, o_min];
        
  

    end

    disp("Observable-specific mixing time was "+num2str(mixingtime))


    %save("variables, n="+num2str(n), "mixingtime", "EvolvedMatrix", "n", "tolerance")


    figure(1)
    plot(times,eig_max,"LineWidth",2,"Color","red","DisplayName","$\lambda_\textup{max}(O(t))$")
    hold on 
    plot(times,eig_min,"LineWidth",2,"Color","red","DisplayName","$\lambda_\textup{min}(O(t))$")
    hold on 
    yline(ExpectationValue,"LineWidth",1.5,"Color","blue",HandleVisibility="off")
    hold on
    yline(ExpectationValue+error_on_observable,"LineWidth",1.5,"Color","black",HandleVisibility="off")
    hold on
    yline(ExpectationValue-error_on_observable,"LineWidth",1.5,"Color","black",HandleVisibility="off")
    hold off
    ax = gca;
    ax.FontSize = 16;
    %title("1D TFIM, J=0.5, h=1, beta=1, Gaussian filter, single-site Paulis","FontSize",36)
    set(gcf,"color","w")
    xlabel("$t$","FontSize",36,"Interpreter","latex")
    ylabel("Spectral range of $O(t)$","FontSize",36,"Interpreter","latex")
    legend("FontSize",24,"Interpreter","latex","Location","northeast")
    grid on  
...
%%%%%% evaluation of the spectral gap
    ...
    if(options.alsoEvaluateGap)
        ...
        norm_here = ctranspose(SteadyVector) * SteadyVector;
    ...
    function [finalvector] = ShiftedEigenvalues(vector)
        ...
        prod = 1/norm_here * ctranspose(SteadyVector) * vector;

        finalvector = finalvector - shiftvalue * vector + shiftvalue/2 * prod * SteadyVector;
    ...
    if(options.alsoEvaluateGap)
        ...
        [eigvector,gap] = eigs(@(sigma) ShiftedEigenvalues(sigma),2^(2*n),1,"largestabs","IsFunctionSymmetric",false);

        eigvector = reshape(eigvector,[2^n,2^n]);
